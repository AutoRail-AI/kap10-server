...
const MCP_SERVER_URL = process.env.MCP_SERVER_URL ?? "https://mcp.kap10.dev"
...
function generatePrBody(repoName: string): string {
  return `## kap10 Code Intelligence

This PR adds kap10 integration to your repository:

- **Bootstrap Rule** (\`.cursor/rules/kap10.mdc\`) — Instructs your AI coding agent to sync context with kap10's knowledge graph before and after code generation
- **MCP Configuration** (\`.cursor/mcp.json\`) — Connects your IDE to kap10's hosted MCP server
...
3. Your AI agent will automatically use kap10's knowledge graph
...
\`\`\`bash
claude mcp add kap10 --transport streamable-http --url ${MCP_SERVER_URL}/mcp --header "Authorization: Bearer <your-api-key>"
\`\`\`
...
*Automatically generated by [kap10](https://kap10.dev)*`
}
