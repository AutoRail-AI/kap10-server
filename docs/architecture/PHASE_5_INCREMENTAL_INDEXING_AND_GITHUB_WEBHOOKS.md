...
8                                      Activity: updateEmbeddings (light-llm-queue)              —
                                       a) For added/modified entities: re-embed via Vertex AI gemini-embedding-001
...
| 7 | **Embedding generation fails** (Vertex AI unavailable) | Low | Entities indexed but not searchable via semantic search. | `IVectorSearch.embed()` throws error. | Skip embedding. Entities are still in ArangoDB (keyword search works). Log to index_event with `embedding_errors`. Next incremental push will retry embedding for those entities. |
