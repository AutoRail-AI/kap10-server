...
**Actor:** Authenticated user (org owner)
**Precondition:** Logged in, has org, zero connected repos
**Outcome:** GitHub App installed for the org → `installation_id` stored
...
sequenceDiagram
    ...
    NextAPI-->>User: Redirect to /repos?connected=true
```
...
- [ ] **P1-INFRA-01: Create GitHub App on github.com** — S
...
- [ ] **P1-INFRA-02: Add GitHub App env vars to `env.mjs`** — S
...
- [ ] **P1-INFRA-03: Update `.env.example` with Phase 1 variables** — S
...
- [ ] **P1-INFRA-04: Increase heavy worker memory limit to 8 GB** — S
...
- [ ] **P1-INFRA-05: Add persistent workspace volume to Docker Compose** — S
...
- [ ] **P1-DB-01: Add `GitHubInstallation` model to Prisma schema** — M
...
- [ ] **P1-DB-02: Extend `Repo` model with indexing fields** — M
...
- [ ] **P1-DB-03: Add ArangoDB index on `[org_id, repo_id, file_path]`** — S
...
- [ ] **P1-ADAPT-01: Implement `GitHubHost.cloneRepo()` with `simple-git`** — M
...
- [ ] **P1-ADAPT-02: Implement `GitHubHost.listFiles()`** — S
...
- [ ] **P1-ADAPT-03: Implement `GitHubHost.createWebhook()`** — S
...
- [ ] **P1-ADAPT-04: Add `getInstallationRepos()` to `IGitHost` and implement** — M
...
- [ ] **P1-ADAPT-05: Add `getInstallationToken()` to `IGitHost` and implement** — S
...
- [ ] **P1-ADAPT-06: Implement `ArangoGraphStore.bulkUpsertEntities()`** — L
...
- [ ] **P1-ADAPT-07: Implement `ArangoGraphStore.bulkUpsertEdges()`** — L
...
- [ ] **P1-ADAPT-08: Implement `ArangoGraphStore.getEntitiesByFile()`** — S
  - Replace no-op stub
...
- [ ] **P1-ADAPT-10: Implement `TemporalWorkflowEngine.startWorkflow()`** — M
...
- [ ] **P1-ADAPT-11: Implement `TemporalWorkflowEngine.getWorkflowStatus()`** — S
...
- [ ] **P1-ADAPT-12: Extend `PrismaRelationalStore` with Phase 1 methods** — M
...
- [ ] **P1-API-01: `GET /api/github/install` — GitHub App installation redirect** — S
...
- [ ] **P1-API-02: `GET /api/github/callback` — GitHub App installation callback** — M
  ...
  - Redirects to `/repos?connected=true`
  - **Test:** Valid callback with known state → installation created, repos created, redirect to /repos.
  - **Depends on:** P1-API-01, P1-DB-01, P1-DB-02, P1-ADAPT-12
  - **Files:** `app/api/github/callback/route.ts`
  - **Acceptance:** Installation and repos persisted. Invalid state → 403. Missing installation_id → 400.
  - Notes: _____

- [ ] **P1-API-03: `GET /api/repos/available` — List installable repos** — S
...
- [ ] **P1-API-04: `POST /api/repos` — Connect repos and trigger indexing** — M
...
- [ ] **P1-API-05: `GET /api/repos/[repoId]/status` — Repo indexing status** — S
...
- [ ] **P1-API-06: `DELETE /api/repos/[repoId]` — Disconnect repo** — M
...
- [ ] **P1-API-07: `POST /api/webhooks/github` — GitHub webhook handler** — M
...
- [ ] **P1-API-08: `GET /api/repos/[repoId]/tree` — File tree** — M
...
- [ ] **P1-API-09: `GET /api/repos/[repoId]/entities` — Entities by file** — S
...
- [ ] **P1-API-10: `GET /api/repos/[repoId]/entities/[entityId]` — Entity detail** — S
...
- [ ] **P1-API-11: Define `indexRepoWorkflow` in Temporal** — L
...
- [ ] **P1-API-12: Implement `prepareWorkspace` activity** — L
...
- [ ] **P1-API-16: Register activities in worker entry points** — S
...
- [ ] **P1-API-17: Define `deleteRepoWorkflow`** — M
...
- [ ] **P1-UI-01: "Connect GitHub" button on empty state + repos page** — M
  - Replace disabled "Connect Repository" button (Phase 0) with active "Connect GitHub" button
  - Clicking navigates to `GET /api/github/install` (which redirects to GitHub)
  - If already connected (installation exists): show "Add Repository" button instead
  ...
  - **Files:** `components/dashboard/empty-state-repos.tsx`, `app/(dashboard)/repos/page.tsx`
  - **Acceptance:** Button states match installation status. Design system compliant (bg-rail-fade, size="sm").
  - Notes: _____

- [ ] **P1-UI-02: Repository picker modal** — M
...
- [ ] **P1-UI-03: Repo card component with status badges** — M
...
- [ ] **P1-UI-04: Repos list page — populated state** — S
...
- [ ] **P1-UI-05: Repo detail page with file tree** — L
...
- [ ] **P1-UI-06: Entity detail panel** — M
...
- [ ] **P1-UI-07: Indexing error state with retry button** — S
