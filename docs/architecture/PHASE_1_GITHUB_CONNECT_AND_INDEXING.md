...
- [x] **P1-API-14: Implement `parseRest` activity** — M
  ...
  - **Enrichment pass (post-implementation):** All three language parsers (TypeScript, Python, Go) now extract additional metadata during parsing:
    - **Doc comments** (`doc` field): Extracted via shared `lib/indexer/doc-extractor.ts` utility. TypeScript: JSDoc `/** */` and `//` blocks. Python: triple-quoted docstrings. Go: consecutive `//` comment blocks. Also extracted in SCIP post-pass (`fillBodiesFromSource`). Comments shorter than 10 characters are filtered as noise.
    - **Function metadata** (`is_async`, `parameter_count`, `return_type`, `complexity`): TypeScript extracts async keyword, param count, return type annotation, and cyclomatic complexity. Python excludes `self`/`cls` from param count, detects `async def`, and parses `->` return annotations. Go extracts params and multi-return types. Complexity is estimated by counting branching constructs (`if`/`for`/`while`/`case`/`&&`/`||`/`?`), baseline = 1.
    - **Import edges with metadata**: TypeScript parser extracts `import` statements as edges with `imported_symbols[]`, `import_type` ("value"|"type"), and `is_type_only` fields. Only internal imports (relative or `@/`/`~/` aliases) create edges; external `node_modules` imports are skipped. Edge metadata is preserved through `toEdgeDocs` via spread.
  - **Files:** `lib/indexer/doc-extractor.ts` (new), `lib/indexer/languages/typescript/tree-sitter.ts`, `lib/indexer/languages/python/tree-sitter.ts`, `lib/indexer/languages/go/tree-sitter.ts`, `lib/temporal/activities/indexing-heavy.ts`
...
- [x] **P1-TEST-04: Entity hashing tests** — S
  ...
  - Enrichment fields (`doc`, `is_async`, `parameter_count`, `return_type`, `complexity`) are metadata — they don't affect entity identity or hash computation
