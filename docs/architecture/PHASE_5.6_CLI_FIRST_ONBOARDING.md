...
> **Phase Feature Statement:** _"I run `npx @autorail/unerr` in my project, it authenticates me, detects my IDE, connects my GitHub repos, analyzes my code, and configures MCP — zero copy-paste, zero dashboard clicks."_
>
...
3. Run `unerr auth login --key <paste>` → manually edit MCP config

This creates friction especially for developers evaluating the product. The new flow:

```
$ npx @autorail/unerr connect
...
└────┬─────┘                          └──────┬───────┘                    └────┬────┘
     ...
     │<──────────────────────────────────────┤                                 │
     │                                       │                                 │
     │  Save to ~/.unerr/credentials.json    │                                 │
     │                                       │                                 │
```
...
The `unerr connect` command is the "golden path" — a single command that handles everything:

```
unerr connect [--server <url>] [--key <apiKey>] [--ide <type>]
```

**Steps:**
1. **Auth check** — If no credentials in `~/.unerr/credentials.json`, runs device auth flow inline
2. **Git detection** — Parses `git remote get-url origin` and `git branch --show-current`
3. **Repo lookup** — Calls `GET /api/cli/context?remote=<url>` to check if repo is on unerr
...
**Credentials stored at:** `~/.unerr/credentials.json` (mode `0o600`):
```json
{
...
Running `npx @autorail/unerr` with no subcommand now launches a full interactive setup wizard. This replaces the previous behavior (showing help text) with the onboarding golden path.
...
$ npx @autorail/unerr
  ...
  ● GitHub connection...
    ? Install unerr GitHub App to connect your repos? › (Y/n)
    Opening browser to install unerr GitHub App...
    Waiting for GitHub App installation... ⠋
    ✓ GitHub App installed for jaswanth

  ● Repository setup...
    ? Select repos to analyze:
    ❯ ◉ jaswanth/unerr-server (TypeScript, private)
  ...
  ✓ Ready! Your AI agent now has access to your codebase graph.
    ...
    Logs: .unerr/logs/setup-2026-02-23.log
```
...
| 4. Repo check | Look up on unerr | Skip to MCP config if already indexed |
...
| 9. Done | Summary + log path | All details in `.unerr/logs/` |
...
3. CLI opens browser to `https://github.com/apps/unerr-dev/installations/new?state=xxx`
...
**Logging:** All API calls and responses are logged to `.unerr/logs/setup-{date}.log`. On any error, the CLI prints the log path for debugging.

**Options:** `npx @autorail/unerr [--server <url>] [--key <apiKey>] [--ide <type>]`

**Existing `connect` command preserved:** `unerr connect` still works as before for users who prefer the explicit subcommand.

### 2.5 UI Changes

The Connect to IDE page (`/repos/{id}/connect`) now has:

1. **Primary CTA**: `npx @autorail/unerr connect` command in a highlighted card
...
```sql
ALTER TABLE unerr.api_keys ALTER COLUMN repo_id DROP NOT NULL;
ALTER TABLE unerr.api_keys ADD COLUMN is_default BOOLEAN NOT NULL DEFAULT false;
```
...
| `packages/cli/src/utils/log.ts` | **New** — File logger to `.unerr/logs/` |
...
- **Credentials file** at `~/.unerr/credentials.json` is created with mode `0o600` (owner-only read/write).
...
**Problem:** For local repositories not on GitHub, the CLI currently zips the code and uploads it to Supabase for the heavy Temporal workers to parse (Phase 5.5's `unerr push`). For a 2 GB enterprise monorepo, this upload takes forever and introduces major security concerns — proprietary source code leaves the developer's machine.

**Solution:** Move the `heavy-compute-rust` binary (from Phase ∞) directly into the CLI package. When a user runs `unerr connect`, the CLI parses the AST, extracts the entity graph, and hashes entities *locally on the developer's machine*. Only a lightweight JSON payload of the graph topology (entity metadata, edges, hashes — no source code text) is sent to the cloud.

```
Local MapReduce Pipeline:

unerr connect (or unerr push --local-parse)
  │
  ▼
CLI binary (unerr-parse-local, Rust or WASM):
...
**Fallback:** If the local Rust binary is unavailable (unsupported platform, download failure), the CLI falls back to the existing zip-upload path (Phase 5.5's `unerr push`). The local-parse path is an optimization, never a gate.

### 7.2 Ephemeral Sandbox Mode

**Problem:** Developers want to test unerr on a quick side-project, but doing so pollutes their org's permanent ArangoDB and pgvector databases with entities that will never be maintained.

**Solution:** Add an `--ephemeral` flag to `unerr connect`. This provisions a temporary, isolated namespace with a strict 4-hour TTL.

```
unerr connect --ephemeral
```
...
Ephemeral Namespace:
  ...
  pgvector: unerr.entity_embeddings WHERE namespace = 'ephemeral_{uuid}'
    └── Filtered by namespace column (not separate table)
    └── Cleanup: DELETE WHERE namespace = 'ephemeral_{uuid}'

  Supabase: unerr.repos WHERE ephemeral = true AND ephemeral_expires_at < now()
...
1. `unerr connect --ephemeral` → Server creates repo with `ephemeral: true`, `ephemeral_expires_at: now() + 4h`
...
5. If user wants to keep: `unerr promote` converts ephemeral → permanent (removes TTL, keeps data)

**Database changes:**
- `unerr.repos`: Add `ephemeral Boolean @default(false)`, `ephemeral_expires_at DateTime?`
...
**Problem:** AI agents (Cursor, Claude Code) frequently overwrite, corrupt, or drop MCP config files (`.cursor/mcp.json`, `.vscode/settings.json`) during updates or user error, breaking the unerr connection silently. Users experience "Why isn't the AI seeing my rules?" without realizing the config was damaged.

**Solution:** When the CLI establishes a connection, install a lightweight integrity watchdog that verifies the IDE's MCP config file and silently repairs it if the unerr server entry is missing or corrupted.
...
```
unerr connect installs:
  .git/hooks/post-checkout  (or appends to existing)
  .git/hooks/post-merge     (or appends to existing)

Hook script (bash):
  #!/bin/sh
  # unerr MCP config integrity check
  unerr config verify --silent 2>/dev/null || true
```

`unerr config verify --silent`:
1. Read `.cursor/mcp.json` (or `.vscode/settings.json` depending on detected IDE)
2. Check if unerr server entry exists with correct URL and API key
3. If missing or corrupted:
   a. Re-inject unerr server config (merge, don't overwrite other entries)
   b. Log: `"unerr: MCP config repaired (server entry was missing)"`
4. If correct: exit silently (zero output)

**Strategy B: Background Watchdog (optional, for `unerr watch` mode)**

When `unerr watch` is running (Phase 5.5), add a periodic config check:
1. Every 60 seconds, verify MCP config integrity
2. If damaged, repair silently and log
3. On repair, send a brief notification to the terminal: `"⚡ unerr: Repaired MCP config (was corrupted by IDE update)"`
...
3. Check for unerr entry in mcpServers:
   Expected: { "mcpServers": { "unerr": { "url": "...", "headers": { ... } } } }
4. If unerr entry missing OR url/headers incorrect:
   a. Preserve ALL other mcpServers entries unchanged
   b. Set/overwrite ONLY the "unerr" key with correct config
   c. Write back to file
5. If unerr entry correct: do nothing
```

**Safety guarantees:**
- Never overwrites non-unerr MCP server entries
- Never runs if `unerr connect` was never run in this repo (checks `.unerr/config.json` exists)
- Git hooks are append-safe (check if hook already contains unerr line before adding)
- Watchdog is opt-in (only active during `unerr watch`)
...
**Solution:** Create an **In-Memory Overlay Graph** for uncommitted changes. When the IDE sends the current dirty buffer to unerr, run a hyper-fast, localized `ast-grep` pass just on those dirty lines. Instead of writing this to the database, project these changes as an "Overlay Mask" over the query router. When the agent asks "What does this function do?", the router prioritizes the dirty in-memory mask over the persistent database.
...
Data Flow:
  ...
  MCP handler:
    ...
    2. Store in Redis ephemeral overlay:
       Key: unerr:dirty:{sessionId}:{filePath}
       Value: {
...
Edge Cases:
  ...
  - unerr watch integration: If unerr watch is running (Phase 5.5), it detects
...
- **Description:** Package the Phase ∞ Rust `unerr-parse-rest` binary (or a subset) for distribution with the CLI. Cross-compile for macOS/Linux/Windows.
...
- **Files:**
  - `workers/heavy-compute-rust/src/bin/unerr-parse-local.rs` (new — CLI-specific entry point)
...
- **Blocked by:** Phase ∞ RUST-07 (unerr-parse-rest binary)
...
- **Description:** Add `--ephemeral` flag to `unerr connect`. Provisions temporary isolated namespace with 4-hour TTL.
- **Database changes:**
  - Add `ephemeral Boolean @default(false)` and `ephemeral_expires_at DateTime?` to `unerr.repos` Prisma model
...
- **CLI changes:**
  - `unerr connect --ephemeral`: Sets ephemeral flag, shows TTL countdown in terminal
  - `unerr promote`: New command to convert ephemeral → permanent
- **Files:**
...
- **Testing:** Ephemeral repo created with TTL. Cleanup runs after expiry. ArangoDB + pgvector + Supabase data deleted. Audit log written. `unerr promote` removes ephemeral flag and TTL. Multiple ephemeral repos don't interfere.
- **Blocked by:** Nothing
...
- **Git hooks:**
  - `post-checkout` and `post-merge` hooks that run `unerr config verify --silent`
  - Append-safe: check for existing unerr line before adding
  - Non-blocking: failures are silent (`|| true`)
- **Config verify command:**
  - `unerr config verify [--silent]`: Check MCP config integrity, repair if needed
  - Merge-safe: only modifies the `unerr` key in `mcpServers`, preserves all others
  - `--silent`: No output on success, brief log on repair
- **Watchdog integration:**
  - `unerr watch` gains a 60-second config integrity check loop
  - On repair: terminal notification `"⚡ unerr: Repaired MCP config"`
- **Files:**
  - `packages/cli/src/commands/config.ts` (new — `unerr config verify`)
...
- **Testing:** Missing unerr entry → repaired. Corrupted JSON → repaired (other entries preserved). Correct config → no changes. Git hooks installed correctly. Hooks don't break existing hooks. Watchdog detects and repairs within 60s. Non-unerr MCP servers never modified.
- **Blocked by:** Nothing
...
- **Test cases:**
  ...
  - `unerr promote` removes ephemeral flag
...
- **Test cases:**
  - Missing unerr entry → repaired correctly
...
| Date | Author | Change |
|------|--------|--------|
| 2026-02-21 | — | Initial document created. CLI-First Zero-Friction Onboarding: Device Auth Flow, org-level API keys, `unerr connect` golden path command, IDE auto-detection. |
