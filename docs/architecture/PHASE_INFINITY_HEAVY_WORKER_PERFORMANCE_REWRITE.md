...
1. **Pre-migration state:** All three activities (`prepareRepoIntelligenceSpace`, `runSCIP`, `parseRest`) are pure TypeScript, running in the same Node.js process.
...
2. **Select activity to migrate.** Priority order based on impact:
   ...
   - `prepareRepoIntelligenceSpace` second (git clone latency, subprocess spawn overhead)
...
Trigger: index-repo workflow reaches the prepareRepoIntelligenceSpace step
```

**Current TypeScript path:**

1. `prepareRepoIntelligenceSpace` calls `container.gitHost.cloneRepo()`.
...
4. After clone: `scanIndexDir()` spawns `git ls-files` as another subprocess (50 MB stdout buffer).
5. `detectPackageRoots()` reads YAML/JSON config files from disk.
...
5. Output: JSON to stdout with `{ indexDir, languages, packageRoots, fileCount }`.
...
│  │                                                      │      │
│  │  ┌─────────────────────────────────────────────────┐ │      │
│  │  │ Activity: prepareRepoIntelligenceSpace             │ │      │
│  │  │  if USE_RUST_PREPARE:                           │ │      │
...
```
unerr-scip-parse \
  --input /data/repo-indices/{orgId}/{repoId}/index.scip \
  --repo-id {repoId} \
  --workspace-dir /data/repo-indices/{orgId}/{repoId} \
...
```
Step 1: prepareRepoIntelligenceSpace  → { indexDir, languages, packageRoots }
...
```
Step 1: prepareRepoIntelligenceSpace  → { indexDir, languages, packageRoots, fileCount }
...
2. Docker compose configuration mounts `repo-indices:/data/repo-indices` — same as current TypeScript worker.
...
The current `cleanup-workspaces` workflow only deletes ArangoDB overlay entities and Prisma workspace records. Cloned repos on disk at `/data/repo-indices/` accumulate forever.

Phase ∞ adds filesystem cleanup to the Rust `prepareRepoIntelligenceSpace` binary:
...
| **Phase 1** (Repo Indexing) | `prepareRepoIntelligenceSpace`, `runSCIP`, `parseRest` activity contracts, `entityHash`, `edgeHash`, KIND_TO_COLLECTION | These are the exact contracts Rust must satisfy. Hash algorithm must be byte-identical. |
...
- [ ] **INFRA-05: Add worker-tuning environment variables**
  - `REPO_INDEX_BASE_PATH` (default: `/data/repo-indices`) — replaces hardcoded path
...
- [ ] **RUST-08: Implement `unerr-workspace-gc` binary**
  ...
  - Pipeline: walk `/data/repo-indices/` → for each `{orgId}/{repoId}/` directory, check last modified time → remove directories older than `max-age-days`
...
- [ ] **WRAP-02: Create TypeScript wrapper for `unerr-prepare-workspace`**
  - Same pattern as WRAP-01, in `prepareRepoIntelligenceSpace` activity
...
- [ ] **DB-01: Extract `REPO_INDEX_BASE_PATH` to environment variable**
  - Replace hardcoded `/data/repo-indices/{orgId}/{repoId}` in `indexing-heavy.ts` with `process.env.REPO_INDEX_BASE_PATH`
  - Add to `env.mjs` with default `/data/repo-indices`
