...
export const POST = withAuth(async (req: NextRequest) => {
  ...
  const match = path.match(/^\/api\/repos\/([^/]+)\/justify/)
  ...
  try {
    // Start discoverOntologyWorkflow which chains to justifyRepoWorkflow
    const handle = await container.workflowEngine.startWorkflow({
      workflowId: `ontology-${orgId}-${repoId}`,
      workflowFn: "discoverOntologyWorkflow",
      args: [{ orgId, repoId }],
