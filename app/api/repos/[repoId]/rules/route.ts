...
export const GET = withAuth(async (req: NextRequest) => {
  ...
  const rules = await container.graphStore.queryRules(orgId, {
    orgId,
    repoId,
    scope: url.searchParams.get("scope") as "org" | "repo" | "path" | "branch" | "workspace" | undefined,
    type: url.searchParams.get("type") as "architecture" | "naming" | "security" | "performance" | "style" | "custom" | undefined,
    status: (url.searchParams.get("status") as "active" | "draft" | "deprecated" | "archived" | undefined) ?? "active",
    limit: 50,
  })
...
export const POST = withAuth(async (req: NextRequest) => {
  ...
  return successResponse({ id: ruleId }, "Rule created", 201)
})
